...
def translate_message(mlist):
	outputlist=[]
	for integer in mlist:
		cint = integer % 72
		outputlist.append(beta[cint])
	delimiter=''
	output_message = delimiter.join(outputlist)
...
def job_output():
	if outputmode==1:
		print translate_message(job())
	else:
		fout=open(outfilename , 'w')
		fout.write(translate_message(job()))
		fout.close()	
